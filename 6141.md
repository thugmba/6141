...
* **Keywords (Core Programming Concepts):**
  * `Vectorized Computation (벡터화 연산)`: Executing mathematical operations across entire numeric arrays or data columns simultaneously in compiled C memory, replacing slow Python row-by-row loops.
  * `Pandas Series (시리즈)`: A one-dimensional labeled array in Pandas capable of holding any data type, representing a single column in an enterprise tabular dataset.
  * `Pandas DataFrame (데이터프레임)`: A two-dimensional, size-mutable tabular data structure with labeled axes (rows and columns), serving as the primary analytical workhorse for commercial data.
  * `Tabular CSV Ingestion (CSV 데이터 적재)`: Reading structured comma-separated corporate ledgers directly into memory using `pd.read_csv()`, parsing data types and column headers automatically.
...
* **Keywords (Core Programming Concepts):**
  * `Boolean Masking (불리언 마스킹)`: Filtering tabular rows using boolean condition vectors combined with bitwise logical operators (`&` for AND, `|` for OR, `~` for NOT).
  * `Split-Apply-Combine Pattern (분할-적용-결합 패턴)`: The computational workflow of splitting data into operational groups, applying analytical functions, and combining results into an aggregated table.
  ...
  * `Relational Merging (테이블 병합 - pd.merge)`: Joining two DataFrames based on common business keys (such as `Cost_Center_Code`), replacing error-prone spreadsheet `VLOOKUP` and `XLOOKUP` functions.
  * `Financial Variance Auditing (재무 차이 감사)`: Programmatically comparing actual transactional expenses against authorized budget baselines to flag cost center overruns.
...
* **Keywords (Core Programming Concepts):**
  ...
  * `Beautiful Soup Parser (HTML 파서)`: A Python library (`bs4`) that navigates, searches, and modifies the Document Object Model tree to extract clean textual data.
  * `Ethical Scraping Protocols (스크래핑 윤리 & robots.txt)`: Operating web crawlers responsibly by respecting server rate limits, adding descriptive User-Agent headers, and complying with `robots.txt` exclusion rules.
...
* **Keywords (Core Programming Concepts):**
  * `Visual Encoding Hierarchy (시각적 부호화)`: Mapping quantitative commercial data to spatial coordinates, lengths, and colors following cognitive visual perception principles.
  * `Figure & Axes Architecture (Figure & Axes 계층)`: The object-oriented Matplotlib canvas model where a top-level `Figure` container manages one or more individual `Axes` plotting subplots.
  * `Time-Series Revenue Curves (시계열 추세선)`: Continuous line plots visualizing revenue, marketing spend, and customer acquisition costs across fiscal months.
  * `Categorical Distribution Plots (범주형 분포 시각화)`: Clean bar plots and box plots created with Seaborn to compare performance metrics across corporate business units.
  * `Correlation Heatmap (상관관계 히트맵)`: A color-encoded matrix displaying pairwise correlation coefficients between multiple operational metrics to uncover commercial relationships.
...
* **Keywords (Core Programming Concepts):**
  * `Analytical Hypothesis Framing (분석 가설 수립)`: Translating ambiguous managerial problems into falsifiable, testable quantitative statements expressed in executable code.
  * `Exploratory Data Analysis (탐색적 데이터 분석 - EDA)`: Systematic statistical inspection of dataset distributions, correlations, and anomalies to inform analytical design.
  * `Data Hygiene Audit (데이터 정합성 감사)`: Automated scanning for missing records, duplicate primary keys, unexpected negative numbers, and data type inconsistencies.
  * `Baseline Metric Architecture (기준 지표 설계)`: Calculating baseline benchmarks (e.g., historical contract renewal rate) against which analytical improvements are measured.
  * `Technical Debt Profiling (코드 부채 진단)`: Auditing student codebase for modular function structure, descriptive variable naming, and automated exception handling.
...
* **Keywords (Core Programming Concepts):**
  * `Reproducible Data Pipeline (재현 가능한 데이터 파이프라인)`: A self-contained, end-to-end Python script execution sequence that reliably transforms raw data into verified executive reports with zero manual intervention.
  * `Executive Code Defense (임원급 코드 구두 방어)`: Articulating the business rationale, methodology, and computational assumptions behind an analytics codebase to executive decision-makers.
  * `Exploratory Data Findings (탐색적 분석 결과 보고)`: Structuring quantitative insights into concise, actionable findings linked directly to operating cash flows and margin expansion.
  * `Cross-Functional Technical Translation (비즈니스 기술 번역)`: The ability to explain computational trade-offs, data limitations, and algorithmic results to non-technical stakeholders.
  * `Peer Review Audit Protocol (동료 코드 감사 프로토콜)`: Performing structured code inspections of classmate scripts to verify PEP 8 compliance, error handling, and numerical reproducibility.
...
* **Keywords (Core Programming Concepts):**
  ...
  * `Machine Learning (머신러닝 - ML)`: A subfield of artificial intelligence where statistical models learn patterns and relationships directly from data rather than following rigid, hand-coded rules.
  * `Deep Learning (딥러닝 - DL)`: A subset of machine learning based on multi-layered artificial neural networks, specialized in unstructured data processing (images, audio, natural language).
...
* **Keywords (Core Programming Concepts):**
  ...
  * `One-Hot Categorical Encoding (원-핫 인코딩)`: Transforming text categories (e.g., Bachelor, Master, Doctorate) into binary numerical indicator columns (`0` or `1`) for machine learning models.
  * `Feature Standardization (표준화 - StandardScaler)`: Rescaling continuous numeric features to have a mean of 0 and a standard deviation of 1, preventing high-magnitude columns from dominating distance calculations.
  * `Missing Value Imputation (결측치 대체 - SimpleImputer)`: Statistically replacing missing data with median or mean values to prevent model training crashes.
  * `Train-Test Partitioning (훈련-테스트 데이터 분할)`: Dividing historical data into an 80% training set to fit the model and a 20% test set held out to evaluate true generalization performance.
...
* **Keywords (Core Programming Concepts):**
  * `Unified Estimator API (일관된 모델 API)`: The standard three-stage Scikit-Learn interface: instantiate model, fit on training data (`.fit()`), predict on new data (`.predict()`), and evaluate accuracy (`.score()`).
  * `Linear Regression (선형 회귀)`: A fundamental supervised regression algorithm that models the linear relationship between continuous independent variables and a target business metric.
  * `Logistic Regression (로지스틱 회귀)`: A foundational supervised classification algorithm that models the probability of a binary categorical event using the logistic sigmoid function.
  * `K-Fold Cross-Validation (K-겹 교차 검증)`: Splitting training data into K subsets to iteratively train and validate the model K times, providing an unbiased estimate of generalization performance.
  * `Root Mean Squared Error (평균 제곱근 오차 - RMSE)`: A standard regression performance metric that penalizes large forecasting errors by taking the square root of mean squared residuals.
...
* **Keywords (Core Programming Concepts):**
  * `Supervised vs. Unsupervised Learning (지도학습 vs 비지도학습)`: Supervised learning trains on labeled historical targets (`y`), whereas unsupervised learning identifies natural groupings and structural patterns in unlabeled data (`X`).
  * `Confusion Matrix (혼동 행렬)`: A 2x2 contingency table tabulating classification outcomes into True Positives (TP), False Positives (FP), True Negatives (TN), and False Negatives (FN).
  * `Precision & Recall Trade-Off (정밀도와 재현율 트레이드오프)`: Precision measures how many flagged cases were truly positive, while Recall measures what percentage of all actual positives were caught.
  ...
  * `K-Means Clustering (K-평균 군집화)`: An unsupervised algorithm that partitions commercial customer accounts into K distinct clusters by iteratively minimizing the sum of squared distances to cluster centroids.
...
* **Keywords (Core Programming Concepts):**
  * `End-to-End Decision Pipeline (엔드투엔드 의사결정 파이프라인)`: A unified software pipeline connecting data ingestion, automated preprocessing, model inference, and operational decision triggers into a single automated system.
  * `Batch vs. Real-Time Inference (배치 vs 실시간 추론)`: Batch inference scores millions of records offline on scheduled schedules (e.g., nightly credit updates), while real-time inference scores individual transactions in milliseconds via APIs.
  * `Model Serialization (모델 직렬화 - joblib)`: Saving trained Scikit-Learn model weights and preprocessing transformers to disk (`.joblib` format) to load into production systems without retraining.
  * `Data Drift & Model Degradation (데이터 드리프트 & 성능 저하)`: The real-world phenomenon where macroeconomic shifts or consumer behavior changes degrade model accuracy over time, requiring continuous monitoring.
  * `Production Health Monitoring (운영 건전성 모니터링)`: Programmatically tracking execution latency, throughput volumes, and prediction distributions to detect pipeline failures.
...
* **Keywords (Core Programming Concepts):**
  * `Customer Churn Probability (고객 이탈 확률)`: The continuous probability output (`predict_proba`) generated by a classification model estimating the likelihood of a subscriber canceling services.
  * `Customer Lifetime Value (고객 생애 가치 - CLV)`: The present value of future net profit margins generated by a customer relationship across their projected relationship tenure.
  * `Revenue At Risk (이탈 위험 노출 매출)`: Quantifying the total annual revenue at risk by multiplying individual churn probabilities by annualized subscriber contract values.
  * `Targeted Retention Incentives (맞춤형 고객 유지 프로모션)`: Programmatically matching customer churn tiers to specific cost-effective intervention strategies (e.g., proactive customer support outreach vs. discount incentives).
  * `Retention Campaign Return on Investment (캠페인 ROI)`: The financial equation comparing saved customer revenue against the operational cost of retention interventions.
...
* **Keywords (Core Programming Concepts):**
  * `Price Elasticity of Demand (가격 탄력성 - PED)`: The percentage change in quantity demanded resulting from a 1% change in product price, quantifying consumer price sensitivity.
  * `Revenue Management Optimization (수익 관리 최적화)`: Programmatically identifying the price point that maximizes total gross profit (`(Price - Unit_Cost) * Quantity`) subject to market demand curves.
  * `Gross Margin Simulation (매출총이익 시뮬레이션)`: Simulating projected sales volume and financial profit across discrete pricing scenarios (+5%, +10%, -10%).
  * `Competitor Price Gap Sensitivity (경쟁사 가격 격차)`: Modeling how the premium or discount relative to competitor catalog prices influences customer purchase conversion.
  * `Constrained Inventory Markdown (재고 제약 가격 인하)`: Algorithmic pricing logic that automatically applies markdown discounts when inventory velocity indicates risk of unsold seasonal stock.
...
* **Keywords (Core Programming Concepts):**
  * `Algorithmic Disparate Impact (알고리즘 불리한 영향)`: A legal and quantitative metric occurring when an automated decision system yields substantially different adverse outcomes across protected demographic groups.
  * `Demographic Parity Metric (인구통계학적 동등성)`: The fairness standard requiring that the likelihood of receiving a positive algorithmic outcome (e.g., credit approval) is equal across all demographic groups.
  * `Model Explainability (모델 설명 가능성 - SHAP / Feature Attribution)`: Algorithmic frameworks that decompose complex black-box model predictions into transparent, auditable feature contributions for adverse action notices.
  * `Regulatory Compliance (규제 준수 - EU AI Act Framework)`: Auditing machine learning systems against emerging international governance standards (such as the European Union Artificial Intelligence Act for high-risk credit and hiring systems).
  * `Boardroom AI Governance (이사회 AI 거버넌스 프레임워크)`: Enterprise governance architecture establishing risk management protocols, algorithmic audit trails, human-in-the-loop overrides, and executive accountability.
